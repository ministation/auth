...
def link_account(session: Session, user_id: uuid.UUID, discord_id: int) -> DiscordAuth:
    ...
    if existing_discord is not None and existing_discord.user_id != user_id:
        raise LinkConflictError("Этот Discord уже привязан к другому игровому аккаунту")
    ...
    if (
        ...
        raise LinkConflictError("Этот игровой аккаунт уже привязан к другому Discord")
    ...
    try:
        ...
        raise LinkConflictError(
            "Этот Discord или игровой аккаунт уже привязан (конфликт уникальности)"
        ) from exc
